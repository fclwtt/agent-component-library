...
def _print_deep_probes() -> None:
    ...
    if lock_present:
        try:
            from components.gateway.api import is_gateway_runtime_lock_active
...
def _drain_gateway_pid(pid: int, drain_timeout: float) -> bool:
    ...
    if pid <= 0:
        return False
    try:
        from components.gateway.api import write_planned_stop_marker, _pid_exists
    except ImportError:
        return False
...
def _force_terminate_known_gateway_pids(pids: list[int]) -> int:
    ...
    try:
        from components.gateway.api import _pid_exists, terminate_pid
    except ImportError:
        return 0
