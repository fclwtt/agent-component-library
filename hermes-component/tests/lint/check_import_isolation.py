...
def check_repository(repo_root: str | Path) -> list[str]:
    root = Path(repo_root)
    comp_dir = root / "hermes_component"
...
def main():
    ...
    syn = [v for v in violations if '语法错误' in v]
    bad = [v for v in violations if v not in syn]
    if bad:
        print(f"FAIL: {len(bad)} isolation violation(s)")
        for v in bad:
            print(f"  {v}")
        sys.exit(1)
    elif syn:
        print(f"WARN: {len(syn)} syntax error(s) (not isolation issues)")
        for v in syn:
            print(f"  {v}")
        print("PASS: import isolation check complete")
        sys.exit(0)
    else:
        print("PASS: import isolation check complete")
        sys.exit(0)
