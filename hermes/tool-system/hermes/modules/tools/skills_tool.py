...
from components.entry-points.api import get_hermes_home, display_hermes_home
import os
import re
...
from components.cli.api import cfg_get
from components.entry-points.api import env_var_enabled
...
def _gateway_setup_hint() -> str:
    try:
        from components.gateway.api import GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE
...
def _parse_frontmatter(content: str) -> Tuple[Dict[str, Any], str]:
    ...
    as a public re-export so existing callers don't need updating.
    """
    from components.agent-engine.api import parse_frontmatter
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    try:
        from components.agent-engine.api import get_external_skills_dirs
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    """
    try:
        from components.cli.api import load_config
        config = load_config()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    ...
    """
    from components.agent-engine.api import get_external_skills_dirs, iter_skill_index_files
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from components.cli.api import _get_disabled_plugins, get_plugin_manager
    ...
    if preprocess:
        try:
            from components.agent-engine.api import preprocess_skill_content
...
def skill_view(
    ...
    try:
        ...
        if ":" in name:
            from components.agent-engine.api import is_valid_namespace, parse_qualified_name
            from components.cli.api import discover_plugins, get_plugin_manager
        ...
        from components.agent-engine.api import iter_skill_index_files
