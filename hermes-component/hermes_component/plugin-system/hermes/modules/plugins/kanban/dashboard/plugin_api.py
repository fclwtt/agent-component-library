...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from components.cli.api import SEVERITY_ORDER
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from components.cli.api import _check_dispatcher_presence
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            from components.entry-points.api import get_hermes_home  # type: ignore
