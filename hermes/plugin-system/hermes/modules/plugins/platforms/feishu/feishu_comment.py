...
def _resolve_model_and_runtime() -> Tuple[str, dict]:
    ...
    from components.gateway.api import _load_gateway_config, _resolve_gateway_model

    user_config = _load_gateway_config()
    model = _resolve_gateway_model(user_config)

    from components.gateway.api import _resolve_runtime_agent_kwargs
    runtime_kwargs = _resolve_runtime_agent_kwargs()

    # Fall back to provider's default model if none configured
    if not model and runtime_kwargs.get("provider"):
        try:
            from components.cli.api import get_default_model_for_provider
