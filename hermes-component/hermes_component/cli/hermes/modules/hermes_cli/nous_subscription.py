...
from components.tool-system.api import is_managed_tool_gateway_ready
...
def _toolset_enabled(config: Dict[str, object], toolset_key: str) -> bool:
    from components.entry-points.api import resolve_toolset
...
def _has_agent_browser() -> bool:
    ...
    from components.entry-points.api import agent_browser_runnable
...
def _local_browser_runnable() -> bool:
    ...
    if not _has_agent_browser():
        return False
    try:
        from components.tool-system.api import _chromium_installed, _using_lightpanda_engine
    except Exception:
...
def _local_stt_backend_available() -> bool:
    ...
    if get_env_value("HERMES_LOCAL_STT_COMMAND"):
        return True
    try:
        from components.tool-system.api import _HAS_FASTER_WHISPER
