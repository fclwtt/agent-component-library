...
from components.tool-system.api import _ENV_VAR_PATTERN
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    """
    from components.tool-system.api import _interpolate_env_vars
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    """
    try:
        from components.tool-system.api import HermesTokenStorage
...
def cmd_mcp_add(args):
    ...
    if url and auth_type == "oauth":
        ...
        oauth_ok = False
        try:
            from components.tool-system.api import get_manager
...
def mcp_command(args):
    ...
    if action == "serve":
        from components.entry-points.api import run_mcp_server
