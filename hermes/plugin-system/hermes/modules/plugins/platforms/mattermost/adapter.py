...
from components.gateway.api import MessageDeduplicator
...
class MattermostAdapter(BasePlatformAdapter):
    ...
    async def _send_url_as_file(
        ...
        from components.tool-system.api import is_safe_url
    ...
    async def _handle_ws_event(self, event: Dict[str, Any]) -> None:
        ...
        for fid in file_ids:
            try:
                ...
                async with self._session.get(
                    ...
                    if resp.status < 400:
                        file_data = await resp.read()
                        from components.gateway.api import cache_image_from_bytes, cache_document_from_bytes
                        if mime.startswith("image/"):
                            local_path = cache_image_from_bytes(file_data, ext or ".png")
                            media_urls.append(local_path)
                            media_types.append(mime)
                        elif mime.startswith("audio/"):
                            from components.gateway.api import cache_audio_from_bytes
        ...
        from components.gateway.api import resolve_channel_prompt
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        from components.gateway.api import resolve_proxy_url, proxy_kwargs_for_aiohttp
