...
def _bridge_pid_is_ours(pid: int, session_path: Path, expected_start) -> bool:
    ...
    """
    from components.gateway.api import _pid_exists
    if not _pid_exists(pid):
        return False
    if expected_start is not None:
        from components.gateway.api import get_process_start_time
    ...
    from components.gateway.api import _read_process_cmdline
...
from components.gateway.api import Platform, PlatformConfig
from components.gateway.api import WhatsAppBehaviorMixin
from components.gateway.api import to_whatsapp_jid
...
from components.entry-points.api import env_int
...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        if WhatsAppAdapter._DEFAULT_BRIDGE_DIR is None:
            from components.gateway.api import resolve_whatsapp_bridge_dir
    ...
    def _text_batch_key(self, event: MessageEvent) -> str:
        ...
        from components.gateway.api import build_session_key
...
def interactive_setup() -> None:
    ...
    """
    from components.cli.api import get_env_value, save_env_value
