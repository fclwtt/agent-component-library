...
from components.entry-points.api import atomic_replace, is_truthy_value
...
def _containing_skills_root(skill_path: Path) -> Path:
    ...
    ``_find_skill`` first).
    """
    from components.agent-engine.api import get_all_skills_dirs
...
def _background_review_write_guard(
    ...
    try:
        from components.agent-engine.api import is_external_skill_path
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    {"path": Path} or None.
    """
    from components.agent-engine.api import get_all_skills_dirs, is_excluded_skill_path
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    ...
    try:
        from components.entry-points.api import get_default_hermes_root
...
def _skill_not_found_error(name: str, suffix: str = "") -> str:
    ...
    """
    from components.agent-engine.api import _resolve_active_profile_name
