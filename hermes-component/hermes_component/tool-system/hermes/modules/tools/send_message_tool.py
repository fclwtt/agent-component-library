...
from components.agent-engine.api import redact_sensitive_text
...
def _handle_list():
    ...
    try:
        from components.gateway.api import format_directory_for_display
...
def _handle_react(args, remove=False):
    ...
    if target_ref:
        ...
        if not chat_id:
            try:
                from components.gateway.api import resolve_channel_name
    ...
    try:
        from components.gateway.api import Platform, load_gateway_config
    ...
    runner = None
    try:
        from components.gateway.api import _gateway_runner_ref
    ...
    try:
        from components.entry-points.api import _run_async
        if remove:
            result = _run_async(
...
def _handle_send(args):
    ...
    try:
        from components.gateway.api import load_gateway_config, Platform
        config = load_gateway_config()
    except Exception as e:
    ...
    if not pconfig or not pconfig.enabled:
        ...
        if platform_name == "weixin":
            ...
            if wx_token and wx_account:
                from components.gateway.api import PlatformConfig
    ...
    from components.gateway.api import BasePlatformAdapter
    ...
    if not chat_id:
        ...
        if not home and platform_name == "weixin":
            ...
            if wx_home:
                from components.gateway.api import HomeChannel
    ...
    try:
        ...
        if isinstance(result, dict) and result.get("success") and mirror_text:
            try:
                from components.gateway.api import mirror_to_session
                from components.gateway.api import get_session_env
...
async def _send_via_adapter(
    ...
    *,
    ...
    entry = None
    try:
        from components.gateway.api import platform_registry
...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None, force_document=False):
    ...
    """
    from components.gateway.api import Platform
    ...
    from components.gateway.api import BasePlatformAdapter, utf16_len

    # Telegram adapter import is optional (requires python-telegram-bot)
    try:
        from components.plugin-system.api import TelegramAdapter
    ...
    if platform == Platform.FEISHU and media_files:
        from components.gateway.api import platform_registry as _pr_feishu
        from components.cli.api import discover_plugins as _dp_feishu
        _dp_feishu()
...
async def _send_telegram(token, chat_id, message, media_files=None, thread_id=None, disable_link_previews=False, force_document=False):
    ...
    try:
        ...
        try:
            from components.gateway.api import resolve_proxy_url
        ...
        if formatted.strip():
            try:
                ...
                if _is_telegram_thread_not_found(md_error) and thread_kwargs:
                    ...
                    if not _has_html:
                        try:
                            from components.plugin-system.api import _strip_mdv2
...
async def _registry_standalone_send(platform_name, pconfig, chat_id, message, thread_id=None):
    ...
    """
    from components.gateway.api import platform_registry
    from components.cli.api import discover_plugins
...
async def _send_signal(extra, chat_id, message, media_files=None):
    ...
    from components.gateway.api import markdown_to_signal
...
async def _send_matrix_via_adapter(pconfig, chat_id, message, media_files=None, thread_id=None):
    ...
    try:
        from components.plugin-system.api import MatrixAdapter
    except ImportError:
...
async def _send_weixin(pconfig, chat_id, message, media_files=None):
    ...
    try:
        from components.gateway.api import check_weixin_requirements, send_weixin_direct
...
async def _send_bluebubbles(extra, chat_id, message):
    ...
    try:
        from components.gateway.api import BlueBubblesAdapter, check_bluebubbles_requirements
...
def _check_send_message():
    ...
    if platform and platform != "local":
        return True
    try:
        from components.gateway.api import is_gateway_running
...
async def _send_yuanbao(chat_id, message, media_files=None):
    ...
    """
    try:
        from components.gateway.api import get_active_adapter, send_yuanbao_direct
    except ImportError:
