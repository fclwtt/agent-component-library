...
from components.agent-engine.api import get_read_block_error
...
def _expand_tilde(path: str) -> str:
    ...
    if not path or "~" not in path:
        return path
    try:
        from components.entry-points.api import get_subprocess_home
...
def _mark_verification_stale(
    ...
    if not paths:
        return
    try:
        from components.agent-engine.api import project_facts_for
        from components.agent-engine.api import mark_workspace_edited
