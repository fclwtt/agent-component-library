...
from components.plugin-system.api import _get_exa_client  # noqa: F401
...
def _env_value(name: str) -> str:
    ...
    """
    try:
        from components.cli.api import get_env_value
...
def _resolve_web_extract_auxiliary(model: Optional[str] = None) -> tuple[Optional[Any], Optional[str], Dict[str, Any]]:
    ...
    if client is not None and _is_nous_auxiliary_client(client):
        from components.agent-engine.api import get_auxiliary_extra_body
        from components.agent-engine.api import nous_portal_tags
...
def _ensure_web_plugins_loaded() -> None:
    ...
    """
    try:
        from components.cli.api import _ensure_plugins_discovered
...
async def web_extract_tool(
    ...
    from components.agent-engine.api import _PREFIX_RE
